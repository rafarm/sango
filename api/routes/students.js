...
router.get('/bygroup/:id/:year', function(req, res) {
    var pipe = [
	{$match:{enrolments:{$elemMatch: {year:req.params.year,group_id:req.params.id,active: true}}}},{$unwind:'$enrolments'},{$project:{first_name: 1,last_name: 1,birth_date: 1,gender: 1,repeats:'$enrolments.repeats',ed_measures:'$enrolments.ed_measures',year:'$enrolments.year',group_id:'$enrolments.group_id',course_id:'$enrolments.course_id',student_subjects:'$enrolments.subjects',course_subjects:'$enrolments.subjects'}},{$lookup:{from:'groups',localField:'group_id',foreignField:'_id',as:'group'}},{$unwind:'$group'},{$unwind:'$course_subjects'},{$sort:{last_name:1,first_name:1}},{$group:{_id:{_id:'$group_id',name:'$group.name',short_name:'$group.short_name',year:'$year',course_id:'$course_id'},subjects:{$addToSet:{_id:'$course_subjects.subject_id',adapted:'$course_subjects.adapted'}},students:{$addToSet:{_id:'$_id',first_name:'$first_name',last_name:'$last_name',birthdate:'$birth_date',gender:'$gender',repeats:'$repeats',ed_measures:'$ed_measures',subjects:'$student_subjects'}}}},{$project:{_id:'$_id._id',name:'$_id.name',short_name:'$_id.short_name',year:'$_id.year',course_id:'$_id.course_id',subjects:1,students:1}}
    ];
